...
from src.LibByzaticCommon.InMemoryStorages.ABCAbstractCollection.KeyValueObjectStorageInterface import KeyValueObjectStorageInterface
...
class KeyValueObjectStorage(KeyValueObjectStorageInterface):
    def __init__(self, storage_name: str, critical_dump_flag: bool = True):
        ...
        Is a descendant of the LibByzaticCommon.NodeStorage.ABCAbstractCollection.KeyValueObjectStorageInterface
        ...
        """
        self.__logger: logging.Logger = logging.getLogger("LibByzaticCommon-NodeStorage-logger")
        self.__storage: dict = {}
        self.__storage_name: str = storage_name
        self.__critical_dump_flag: bool = critical_dump_flag
    ...
    def read(self, key: str) -> dict:
        ...
        @param key: string identification parameter
        @return: object if success or raise OperationIncompleteException
        """
        try:
            ops_name = str(inspect.currentframe().f_code.co_name)
            self.__logger.debug(f"storage: {self.__storage_name} "
    ...
    def __check_if_value_empty(self, value: any, ops_name: str) -> None:
        try:
            if len(value) == 0:
                self.__logger.debug(f"storage: {self.__storage_name} "
                                    f"operation: {ops_name} "
